...
class User
{
    ...
    {
        $db = getConnection();
        $statement = $db->prepare("SELECT * FROM tbl_roles where role_id =?");
    ...
    public function select_2($personneId)
    {
        ...
        try {
            $sql =  "SELECT * FROM tbl_users as u,tbl_roles as r WHERE u.role_id =r.role_id and u.role_id!='' and  personne_id=:personne_id";
            $stmt = $db->prepare($sql);
            $stmt->bindParam("personne_id", $personneId);
            $stmt->execute();
    ...
    {
        $db = getConnection();
        $add = $db->prepare("INSERT INTO tbl_staff (noms,tel,sexe,adress,role,etat,branche_id,personne_id) VALUES (?,?,?,?,?,?,?,?)");
    ...
    {
        $db = getConnection();
        $add = $db->prepare("SELECT * FROM tbl_staff, tbl_salaire, tbl_devise,tbl_branches 
        WHERE tbl_staff.staff_id = tbl_salaire.staff_id
        and tbl_devise.devise_id = tbl_salaire.devise_id 
        and tbl_staff.branche_id = tbl_branches.branche_id
        and tbl_staff.etat='1'");
        $add->execute();
        $tbP = array();
    ...
    public function getStaffBrancheSalaire($branche_id)
    {
        $db = getConnection();
        $add = $db->prepare("SELECT * FROM tbl_staff, tbl_salaire, tbl_devise,tbl_branches 
        WHERE tbl_staff.staff_id = tbl_salaire.staff_id
        and tbl_devise.devise_id = tbl_salaire.devise_id 
        and tbl_staff.branche_id = tbl_branches.branche_id
        and tbl_staff.etat='1' and tbl_staff.branche_id = ?");
        $add->execute([$branche_id]);
        $tbP = array();
        while ($data =  $add->fetchObject()) {
            $tbP[] = $data;
        }
    ...
    {
        $db  = getConnection();
        $stmt = $db->prepare("SELECT * FROM tbl_users  where role_id=8 and noms like '%" . $keyword . "%'");
        $stmt->execute();
    ...
    public function getStaffBranche($id)
    {
        $db = getConnection();
        $add = $db->prepare("SELECT * FROM tbl_staff as s,tbl_branches as b WHERE b.branche_id = s.branche_id and b.branche_id =?");
        $add->execute([$id]);
        $tbP = array();
        while ($data =  $add->fetchObject()) {
            $tbP[] = $data;
