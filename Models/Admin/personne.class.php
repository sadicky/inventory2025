...
class Personne
{
    ...
    public function select_1($personneId)
    {
        ...
        $db = getConnection();
        try {
            $sql =  "SELECT * FROM tbl_personnes as p, tbl_users as u WHERE u.personne_id = p.personne_id and user_id=:id";
            $stmt = $db->prepare($sql);
            $stmt->bindParam("id", $personneId);
            $stmt->execute();
    ...
    public function insert($role, $nom, $contact, $email, $genre, $adresse)
    {
        $db = getConnection();

        $stmt = $db->prepare("INSERT INTO tbl_personnes (role,nom_complet,contact,email,genre,adresse)
             VALUES(?,?,?,?,?,?)");
        $stmt->execute(array($role, $nom, $contact, $email, $genre, $adresse)) or die(print_r($stmt->errorInfo()));

        $id = $db->lastInsertId();

        return $id;
    }
    public function insert_2($role, $nomComplet)
    {
        $db = getConnection();
        try {
            $sql = "INSERT INTO tbl_personnes  (role,nom_complet)  VALUES(:role,:nom)";
