...
if ($_SESSION['role'] == 1) {
	$data = $pos->getStores();
	$getCaisse = $autres->getReparations_encours();
} else {
	$st = $pos->getStoreId($_SESSION['pos']);
	$data = $pos->getBranchePOS($st->branche_id);
	$getCaisse = $autres->getReparation_encours_by_branche($st->branche_id);
}
...
<div class="card" style="margin: 20px;">
	<div class="panel-header bg-light" style="margin: 20px;">
		<div style="float:left; margin-left: 15px; padding-top:10px;">
			<h2>Réparations Encours</h2>
		</div>
		<div style="float:right; margin-right: 15px;padding-bottom:13px;">
			<a href="javascript:void(0)" data-id="<?= @$_GET['id'] ?>" id="ajoutcaissee" class="btn btn-primary btn-sm"> Nouvelle</a>
			<a href="javascript:void(0)" data-id="<?= @$_GET['id'] ?>" id="repayer" class="btn btn-primary btn-sm">Payées</a>
			<a href="javascript:void(0)" data-id="<?= @$_GET['id'] ?>" id="repeffectue" class="btn btn-warning btn-sm">Effectuées</a>
			<a href="javascript:void(0)" data-id="<?= @$_GET['id'] ?>" id="repannuler" class="btn btn-danger btn-sm">Annuler</a>
		</div>
	</div>
	<div class="card-body">
					...
					foreach ($getCaisse as $vente) : ?>
						<tr class="odd gradeX">
							...
							<?php if ($_SESSION['role'] == 1 or $_SESSION['role'] == 2 or $_SESSION['role'] == 4) { ?>
								<?php
								if ($vente->statut == 0) {
									echo "<td><a href='javascript:void(0)' data-id='" . $vente->reparation_id . "' class='fs-btn btn-sm btn-primary activers'><i class='fa fa-check' ></i> Effectué?</a></td>";
									echo "<td><a href='javascript:void(0)' data-id='" . $vente->reparation_id . "' class='fs-btn btn-sm btn-danger annulerrep'><i class='fa fa-times' ></i> Annuler</a></td>";
								} else {
