...
if ($_SESSION['role'] == 1) {
	$data = $pos->getStores();
	$getCaisse = $autres->getReparations_payer();
} else {
	$st = $pos->getStoreId($_SESSION['pos']);
	$data = $pos->getBranchePOS($st->branche_id);
	$getCaisse = $autres->getReparation_payer_by_branche($st->branche_id);
}
				...
				<tbody>
					...
					foreach ($getCaisse as $vente) : ?>
						<tr class="odd gradeX">
							...
							<?php if ($_SESSION['role'] == 1 or $_SESSION['role'] == 2 or $_SESSION['role'] == 4) { ?>
								<?php
								if ($vente->statut == 0 and $vente->statutr == 'effectué') {
									echo "<td><a href='javascript:void(0)' data-id='" . $vente->reparation_id . "' class='fs-btn btn-sm btn-primary activer_payer'><i class='fa fa-check' ></i> Validé?</a></td>";
									...
									echo "<td><label class='label label-success'>Payée</label></td>";
									echo "<td>Validé</td>";
								}
								?>
							<?php } ?>
