...
// var_dump($bq);die();
// if($_POST['status']==1) $partyCode='0'; else 
...
if ($trans->select_all_op_count($_POST['op_id']) == 0) {
    ...
    if ($_POST['status']==0) {
        ...
        $id_bq = $bq->caisse_id;
        $pos_id = $_SESSION['pos'];
        $create_date = @$jour->start_date;
        $status = 0;
        $partyCode=$op->party_code;
        $mode_paie = "DETTE";

        $trans_id = $trans->insert($jourId, $OpId, $transaction_type, $descript, $amount, $partyCode, $id_bq, $pos_id, $personne_id, $status, $create_date, $mode_paie);
